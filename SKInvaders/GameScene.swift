...
class GameScene: SKScene, SKPhysicsContactDelegate {
  ...
  // Private GameScene Properties
   ...
   var contentCreated = false
    ...
    var difficultyLevel: CGFloat = 1.0
    // 3
    var timePerMove: CFTimeInterval = 0.0001
  ...
  func createContent() {
    ...
    func setupHud() {
        ...
        // 3
        println(size.height)
        scoreLabel.position = CGPoint(x: 30, y: size.height - (20 + scoreLabel.frame.size.height/2))
        addChild(scoreLabel)
        ...
        // 6
        healthLabel.position = CGPoint(x: 30, y: size.height - (40 + healthLabel.frame.size.height/2))
  ...
  // Scene Update Helpers
    func moveInvadersForUpdate(currentTime: CFTimeInterval) {
        ...
        enumerateChildNodesWithName(kInvaderName) {
            ...
            case .Right:
                node.position = CGPoint(x: node.position.x + self.difficultyLevel, y: node.position.y)
            case .Left:
                node.position = CGPoint(x: node.position.x - self.difficultyLevel, y: node.position.y)
  ...
  // Game End Helpers
    ...
    func nextGame() {
        ...
        // 3
        let startLevelScene: StartGameScene = StartGameScene(size: self.size)
        
        view!.presentScene(startLevelScene, transition: SKTransition.doorsOpenHorizontalWithDuration(1.0))
    }
